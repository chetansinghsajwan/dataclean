...
- **The Base Class:** There is a base class for all cleaners called
  `BaseCleaner`.
- **Immutability & Compilation:** The cleaner is always frozen and follows the
  ...
  creation. During runtime (e.g., in `clean_value`), repeated code and condition
...
Always execute commands exactly as formatted below to prevent lockfile or
environment drift:

- **Environment Setup:** `devbox run task setup` (Installs all dependencies and
  sets up the virtual environment via Devbox + uv).
