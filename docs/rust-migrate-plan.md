...
pub trait Cleaner: Send + Sync {
    ...
    fn clean_column(&self, input: &ArrayRef)
        -> Result<ArrayRef, DatacleanError> {
...
pipeline = Pipeline(
    cleaners=[
        EmailCleaner(),
        PhoneCleaner(default_regions=("IN",)),  # explicit, optional per column
    ],
    # default True — fills gaps for columns with no explicit cleaner
    auto_detect=True,
)
cleaned_df = pipeline.fit_transform(df)
```

- Explicit `cleaners` always wins per-column; `auto_detect` fills the rest — no
  ambiguity.
...
- Registry checked in order; first `detect() == True` wins — same plugin pattern
  as `DataFrame.supports()` today.
...
- [ ] Full spec for `Pipeline.fit` vs `.transform` semantics (schema caching for
